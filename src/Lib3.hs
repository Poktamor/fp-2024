...
marshallState state = Batch submarineQueries
  where
    submarineQueries = map (\(subType, subName, ballastCount) -> Lib2.CreateSubmarine subType subName ballastCount) (Lib2.submarines state)
